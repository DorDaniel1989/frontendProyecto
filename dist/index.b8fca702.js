...
_leafletDefault.default.tileLayer('http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
    attribution: 'Map data &copy; <a href="http://openstreetmap.org">OpenStreetMap</a> contributors, <a href="http://creativecommons.org/licenses/by-sa/2.0/">CC-BY-SA</a>, Imagery © <a href="http://cloudmade.com">CloudMade</a>',
    maxZoom: 18
}).addTo(map);
_leafletDefault.default.control.scale().addTo(map);
const URL = "http://10.10.17.126/api";
//**************************************************************** */
...
// headersA.append('Access-Control-Allow-Origin', 'http://localhost:5000');
...
//   })
fetch('http://10.10.17.126/api/TiempoItems').then((response)=>response.json()
    ...
    if (localStorage.getItem('testObject') === null) {
        for(i = 0; i < registrosTiempo.length; i++)if (registrosTiempo[i].municipio == "donostia" || registrosTiempo[i].municipio == "bilbao" || registrosTiempo[i].municipio == "gasteiz") crearFicha(registrosTiempo[i]);
        //crear el objeto de opciones y lo inserta parseado
        //sacamos el objeto y lo parseamos de vuelta
        window.objeto = localStorage.getItem('testObject');
        window.llaves = Object.keys(JSON.parse(objeto));
        console.log(objeto);
        console.table(JSON.parse(objeto));
    } else {
        var testObjectReturn = localStorage.getItem('testObject');
        testObjectReturn = JSON.parse(testObjectReturn);
        getFichaDesdeApi(testObjectReturn.cero);
        getFichaDesdeApi(testObjectReturn.uno);
        getFichaDesdeApi(testObjectReturn.dos);
    }
    //Genero el contenido de la etiqueta select
    for(i = 0; i < registrosTiempo.length; i++)document.getElementById('opcionLocalidad').innerHTML += `<option value="${registrosTiempo[i].municipio}">${registrosTiempo[i].municipio}</option>`;
});
function registrarUsuario() {
    ...
    headers.append('Access-Control-Allow-Origin', '10.10.17.126:80');
    headers.append('Access-Control-Allow-Credentials', 'true');
    console.log(JSON.stringify(user));
    fetch(`http://10.10.17.126/api/Users`, {
        method: 'POST',
        headers: headers,
...
function loguear(user, pass) {
    ...
    headersL.append('Access-Control-Allow-Origin', 'http://10.10.17.126:5000');
    headersL.append('Access-Control-Allow-Credentials', 'true');
    fetch(`http://10.10.17.126/api/Users/authenticate`, {
        method: 'POST',
        headers: headersL,
        body: JSON.stringify({
            "username": user,
        ...
        //setTimeout(spinerStop, 1000, validacion);
        // document.getElementById('form2').innerHTML += "<div class='spinner-border'></div>"
        console.log(validacion.token);
        console.log(validacion);
        token = validacion.token;
        if (token.length > 30) $('.loginOculto').css("display", "none");
        return false;
    });
}
//Se conecta a la api con ids
function getFichaDesdeApi(nombreMunicipio) {
    fetch(`http://localhost:5000/api/TiempoItems/${nombreMunicipio}`, {
        headers: {
...
function crearFicha(objetoJson) {
    if (!comprobarNumeroMaximoDeCartasLocalStorage()) console.log("no se puede insertar mas elementos");
    else {
        let ficha = `<div class="card  " id="${objetoJson.municipio}" >
    <div class="card-body"> 
        <h5 class="card-title">${objetoJson.municipio}</h5><div class="borrarCartas">X</div>
    ...
    <ul class="list-group list-group-flush droppable" id="ul_${objetoJson.municipio}">
        ...
        infoMunicipio = {
            municipio: `${objetoJson.municipio}`,
            temperatura: `${objetoJson.temperatura}`,
            tempMax: `${objetoJson.tempMax}`,
            tempMin: `${objetoJson.tempMin}`,
            tempMedia: `${objetoJson.tempMedia}`,
            humedad: `${objetoJson.humedad}`,
            precipitacionAcumulada: `${objetoJson.precipitacionAcumulada}`,
            velocidadViento: `${objetoJson.velocidadViento}`,
        ...
        localStorage.setItem(`${objetoJson.municipio}`, JSON.stringify(infoMunicipio));
        $(`.borrarCartas`).click(function() {
            $(this).parent().parent().remove();
            restarCartaLocalStorage();
        });
        //Asignamos la funcion al elemento uando se crea
        $(`.droppable`).droppable({
            drop: function(event, ui) {
                var idParametro = ui.draggable[0].id;
                $idCarta = $(this).parent().attr("id");
                var objetoLS = JSON.parse(localStorage.getItem(`${$idCarta}`));
                switch(idParametro){
                    case "precipitaciones":
                        if (!JSON.parse(localStorage.getItem(`${$idCarta}`)).opciones.precipitacion) {
                            $(this).append(`<li class="list-group-item borrable" id="${$idCarta}"><img class="icono" src="./images/drop32.png" alt=>${JSON.parse(localStorage.getItem(`${$idCarta}`)).precipitacionAcumulada} ml <div class="borrarInfo">X</div></li>`);
                            objetoLS.opciones.precipitacion = true;
                            localStorage.setItem(`${$idCarta}`, JSON.stringify(objetoLS));
                        }
                        break;
                    case "velocidadViento":
                        if (!JSON.parse(localStorage.getItem(`${$idCarta}`)).opciones.velocidadViento) {
                            $(this).append(`<li class="list-group-item borrable" id="${$idCarta}"><img class="icono" src="./images/wind32.png" alt=>${JSON.parse(localStorage.getItem(`${$idCarta}`)).velocidadViento} Km/h <div class="borrarInfo">X</div></li>`);
                            objetoLS.opciones.velocidadViento = true;
                            localStorage.setItem(`${$idCarta}`, JSON.stringify(objetoLS));
                        }
                        break;
                    case "humedad":
                        if (!JSON.parse(localStorage.getItem(`${$idCarta}`)).opciones.humedad) {
                            $(this).append(`<li class="list-group-item borrable" id="${$idCarta}"><img class="icono" src="./images/humedad32.png" alt=>${JSON.parse(localStorage.getItem(`${$idCarta}`)).humedad}  <div class="borrarInfo">X</div></li>`);
                            objetoLS.opciones.humedad = true;
                            localStorage.setItem(`${$idCarta}`, JSON.stringify(objetoLS));
                        }
                        break;
                    case "tempMax":
                        if (!JSON.parse(localStorage.getItem(`${$idCarta}`)).opciones.tempMax) {
                            $(this).append(`<li class="list-group-item borrable" id="${$idCarta}"><img class="icono" src="./images/temperatura-alta32.png" alt=>${JSON.parse(localStorage.getItem(`${$idCarta}`)).tempMax}  <div class="borrarInfo">X</div></li>`);
                            objetoLS.opciones.tempMax = true;
                            localStorage.setItem(`${$idCarta}`, JSON.stringify(objetoLS));
                        }
                        break;
                    case "tempMin":
                        if (!JSON.parse(localStorage.getItem(`${$idCarta}`)).opciones.tempMin) {
                            $(this).append(`<li class="list-group-item borrable" id="${$idCarta}"><img class="icono" src="./images/frio32.png" alt=>${JSON.parse(localStorage.getItem(`${$idCarta}`)).tempMin}  <div class="borrarInfo">X</div></li>`);
                            objetoLS.opciones.tempMin = true;
                            localStorage.setItem(`${$idCarta}`, JSON.stringify(objetoLS));
                        }
                        break;
                    case "tempMedia":
                        if (!JSON.parse(localStorage.getItem(`${$idCarta}`)).opciones.tempMedia) {
                            $(this).append(`<li class="list-group-item borrable" id="${$idCarta}"><img class="icono" src="./images/tempMedia32.png" alt=>${JSON.parse(localStorage.getItem(`${$idCarta}`)).tempMedia.substring(0, 3)} <div class="borrarInfo">X</div></li>`);
                            objetoLS.opciones.tempMedia = true;
                            localStorage.setItem(`${$idCarta}`, JSON.stringify(objetoLS));
                        }
                        break;
                }
        ...
        try {
            latitud = objetoJson.latitud.toString().replace(',', '.');
            longitud = objetoJson.longitud.toString().replace(',', '.');
            const marker = new _leafletDefault.default.marker([
                parseFloat(latitud),
                parseFloat(longitud)
            ]);
            marker.bindPopup(objetoJson.municipio).addTo(map);
...
//Crear fichas con el boton
document.getElementById('añadir').addEventListener('click', recogerDatos);
function recogerDatos() {
    var opcion = $("#opcionLocalidad").val();
    fetch(`http://10.10.17.126/api/TiempoItems/${opcion}`, {
        headers: {
            'Content-Type': 'application/json',
...
function activarReloj() {
    ...
    switch(diaHoyCastellano){
        case 0:
            diaHoyCastellano = "Domingo";
            break;
        case 1:
            diaHoyCastellano = "Lunes";
            break;
        case 2:
            diaHoyCastellano = "Martes";
            break;
        case 3:
            diaHoyCastellano = "Miercoles";
            break;
        case 4:
            diaHoyCastellano = "Jueves";
            break;
        case 5:
            diaHoyCastellano = "Viernes";
            break;
        case 6:
            diaHoyCastellano = "Sabado";
            break;
    }
    switch(mesActual){
        case 0:
    ...
    document.getElementById('minuto').innerHTML = minutoActual;
    //ACTUALIZAR SEGUNDOS, DE MOMENTO SOLO ACTUALIZA ESO JAJAA
    setInterval(incrementar, 60000);
...
},{"jquery":"hgMhh","leaflet":"4FD9G","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"hgMhh":[function(require,module,exports) {
/*!
...
},{}],"4FD9G":[function(require,module,exports) {
module.exports = L;
...
},{}]},["d3wGw","3cYfC"], "3cYfC", "parcelRequire10c2")
